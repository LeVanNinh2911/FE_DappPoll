...
import { Da as hexToNumber, aa as isAddress, yt as isAddressEqual } from "./ccip-D4p2e5rB.js";
import "./_esm-CRgaECpT.js";
import { s as bytesToHex } from "./utils-Dngmz2iM.js";
...
import { a as _, i as B, n as h, r as y } from "./hooks.module-B4rj-foG.js";
import { t as require_sha3 } from "./sha3-CEhN0zL2.js";
...
function ensureAddressString(str) {
	if (typeof str === "string") {
		...
		if (isHexString$1(s) && s.length === 40) return AddressString(prepend0x(s));
	}
...
function ensureIntNumber(num) {
	...
	if (typeof num === "string") {
		...
		if (isHexString$1(num)) return IntNumber(Number(BigInt(ensureEvenLengthHexString(num, true))));
	}
...
function ensureBigInt(val) {
	...
	if (typeof val === "string") {
		...
		if (isHexString$1(val)) return BigInt(ensureEvenLengthHexString(val, true));
	}
...
function getInjectedEthereum() {
	...
	try {
		...
		return (_b = (_a = window$1.top) === null || _a === void 0 ? void 0 : _a.ethereum) !== null && _b !== void 0 ? _b : window$1.ethereum;
	} catch (_c) {
		return;
	}
}
...
var SCWSigner = class {
	...
	*/
	...
	async handleGetCapabilitiesRequest(request) {
		...
		if (!this.accounts.some((account) => isAddressEqual(account, requestedAccount))) throw standardErrors.provider.unauthorized("no active account found");
	...
	updateChain(chainId, newAvailableChains) {
		...
		const chain = chains === null || chains === void 0 ? void 0 : chains.find((chain$1) => chain$1.id === chainId);
...
var require_util = /* @__PURE__ */ __commonJS({ "node_modules/@coinbase/wallet-sdk/dist/vendor-js/eth-eip712-util/util.cjs": ((exports, module) => {
...
var require_abi = /* @__PURE__ */ __commonJS({ "node_modules/@coinbase/wallet-sdk/dist/vendor-js/eth-eip712-util/abi.cjs": ((exports, module) => {
	...
	function encodeSingle(type, arg) {
		...
		if (type === "address") return encodeSingle("uint160", parseNumber(arg));
			...
			if (arg.length % 32 !== 0) ret = Buffer.concat([ret, util$1.zeros(32 - arg.length % 32)]);
			return ret;
	...
	function solidityPack(types, values) {
		...
		for (var i = 0; i < types.length; i++) {
			...
			else if (type === "address") ret.push(util$1.setLength(value, 20));
			else if (type.startsWith("bytes")) {
				size = parseTypeN(type);
				if (size < 1 || size > 32) throw new Error("Invalid bytes<N> width: " + size);
...
//#endregion
//#region node_modules/@coinbase/wallet-sdk/dist/vendor-js/eth-eip712-util/index.cjs
var require_eth_eip712_util = /* @__PURE__ */ __commonJS({ "node_modules/@coinbase/wallet-sdk/dist/vendor-js/eth-eip712-util/index.cjs": ((exports, module) => {
	...
	var TypedDataUtils = {
			...
			if (useV4) {
				const encodeField = (name, type, value) => {
					...
					if (type.lastIndexOf("]") === type.length - 1) {
						...
						return ["bytes32", util.keccak(abi.rawEncode(typeValuePairs.map(([type$1]) => type$1), typeValuePairs.map(([, value$1]) => value$1)))];
					}
					return [type, value];
				};
		...
		encodeType(primaryType, types) {
			...
			for (const type of deps) {
				...
				result += type + "(" + types[type].map(({ name, type: type$1 }) => type$1 + " " + name).join(",") + ")";
			}
			return result;
		},
...
var import_eth_eip712_util = /* @__PURE__ */ __toESM(require_eth_eip712_util(), 1);
...
(function(ConnectionState$1) {
	...
	ConnectionState$1[ConnectionState$1["CONNECTING"] = 1] = "CONNECTING";
	ConnectionState$1[ConnectionState$1["CONNECTED"] = 2] = "CONNECTED";
	...
	async connect() {
		...
		return new Promise((resolve, reject) => {
			...
			webSocket.onclose = (evt) => {
				...
				(_a$1 = this.connectionStateListener) === null || _a$1 === void 0 || _a$1.call(this, ConnectionState.DISCONNECTED);
			};
			webSocket.onopen = (_$1) => {
				var _a$1;
				resolve();
				(_a$1 = this.connectionStateListener) === null || _a$1 === void 0 || _a$1.call(this, ConnectionState.CONNECTED);
			...
			webSocket.onmessage = (evt) => {
				var _a$1, _b;
				if (evt.data === "h") (_a$1 = this.incomingDataListener) === null || _a$1 === void 0 || _a$1.call(this, { type: "Heartbeat" });
				else try {
...
var WalletLinkConnection = class {
	...
	*/
	...
	async makeRequest(message, options = { timeout: REQUEST_TIMEOUT }) {
		...
		let timeoutId;
		return Promise.race([new Promise((_$1, reject) => {
...
var WalletLinkSession = class WalletLinkSession {
	constructor(storage$1, id, secret, linked = false) {
	...
	static create(storage$1) {
		return new WalletLinkSession(storage$1, randomBytesHex(16), randomBytesHex(32)).save();
	}
	static load(storage$1) {
		const id = storage$1.getItem(STORAGE_KEY_SESSION_ID);
		const linked = storage$1.getItem(STORAGE_KEY_SESSION_LINKED);
		const secret = storage$1.getItem(STORAGE_KEY_SESSION_SECRET);
		if (id && secret) return new WalletLinkSession(storage$1, id, secret, linked === "1");
		return null;
	}
	get linked() {
		return this._linked;
...
const SnackbarContainer = (props) => _("div", { class: clsx("-cbwsdk-snackbar-container") }, _("style", null, Snackbar_css_default), _("div", { class: "-cbwsdk-snackbar" }, props.children));
...
var WalletLinkRelay = class WalletLinkRelay {
	...
	publishWeb3RequestEvent(id, request) {
		...
		this.publishEvent("Web3Request", message, true).then((_$1) => {}).catch((err) => {
	...
	handleWeb3ResponseMessage(id, response) {
		if (response.method === "requestEthereumAccounts") {
			WalletLinkRelay.accountRequestCallbackIds.forEach((id$1) => this.invokeCallback(id$1, response));
			WalletLinkRelay.accountRequestCallbackIds.clear();
			return;
		}
...
//#endregion
export { CoinbaseWalletSDK, createCoinbaseWalletSDK, dist_default as default };
//# sourceMappingURL=dist-CrLTgxb6.js.map
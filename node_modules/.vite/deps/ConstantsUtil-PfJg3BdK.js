...
* shadowRoot.
*/
const adoptStyles = (renderRoot, styles$1) => {
	if (supportsAdoptingStyleSheets) renderRoot.adoptedStyleSheets = styles$1.map((s) => s instanceof CSSStyleSheet ? s : s.styleSheet);
	else for (const s of styles$1) {
...
var ReactiveElement = class extends HTMLElement {
	...
	* @category styles
	*/
	static finalizeStyles(styles$1) {
		const elementStyles$1 = [];
		if (Array.isArray(styles$1)) {
			const set = new Set(styles$1.flat(Infinity).reverse());
			for (const s of set) elementStyles$1.unshift(getCompatibleStyle(s));
		} else if (styles$1 !== void 0) elementStyles$1.push(getCompatibleStyle(styles$1));
		return elementStyles$1;
	}
	/**
...
var getTemplateHtml = (strings, type) => {
	...
	return [trustFromTemplateString(strings, html$1 + (strings[l] || "<?>") + (type === SVG_RESULT ? "</svg>" : type === MATHML_RESULT ? "</math>" : "")), attrNames];
};
...
const ThemeHelperUtil = {
	...
	generateW3MOverrides(themeVariables) {
		...
		if (themeVariables["--w3m-font-family"]) overrides["--apkt-fontFamily-regular"] = themeVariables["--w3m-font-family"];
		if (themeVariables["--w3m-z-index"]) overrides["--apkt-tokens-core-zIndex"] = `${themeVariables["--w3m-z-index"]}`;
		return overrides;
	},
	...
	applyColorMixToVariables(themeVariables, allVariables) {
		...
		if (!themeVariables?.["--w3m-color-mix"] || !themeVariables["--w3m-color-mix-strength"]) return colorMixVariables;
		const colorMix = themeVariables["--w3m-color-mix"];
		const strength = themeVariables["--w3m-color-mix-strength"];
...
function initializeTheming(themeVariables, themeMode = "dark") {
	...
	if (!themeVariables?.["--w3m-font-family"]) for (const [key, url] of Object.entries(fonts)) {
...
function setThemeVariables(_themeVariables) {
	...
	if (themeTag && darkModeTag && lightModeTag) {
		...
		if (_themeVariables?.["--w3m-font-family"]) {
			const fontFamily$1 = _themeVariables["--w3m-font-family"];
...
function createRootStyles(_themeVariables) {
	return {
		core: css$1`
      ${Boolean(_themeVariables?.["--w3m-font-family"]) ? css$1`` : css$1`
            @font-face {
              font-family: 'KHTeka';
              src:
...
//#endregion
export { svg as _, initializeTheming as a, css$1 as b, setThemeVariables as c, LitElement as d, _$LH as f, nothing as g, noChange as h, elementStyles as i, css as l, mathml as m, customElement as n, resetStyles as o, html as p, UiHelperUtil as r, setColorTheme as s, ConstantsUtil as t, vars as u, defaultConverter as v, notEqual as y };
//# sourceMappingURL=ConstantsUtil-PfJg3BdK.js.map
import { $i as slice, Ai as formatUnits, Ca as toHex, Da as hexToNumber, Fa as pad, Ii as decodeAbiParameters, Oa as hexToString, Sa as stringToHex, Ta as hexToBigInt, Wi as encodeFunctionData, Yi as encodeAbiParameters, _a as stringToBytes, aa as isAddress, c as decodeFunctionData, do as BaseError, ha as hexToBytes$1, mo as isHex, pa as keccak256, po as size, sa as getAddress, xa as numberToHex, yt as isAddressEqual } from "./ccip-D4p2e5rB.js";
import { En as encodePacked, F as createPublicClient, Kn as waitForCallsStatus, Y as multicall$1, Yt as hashTypedData, dr as readContract, h as http, i as parseSignature, kn as getCode, mn as defineChain, pr as decodeEventLog, rn as hashMessage, rt as parseUnits } from "./_esm-CRgaECpT.js";
import "./utils-Dngmz2iM.js";
import "./sha3-BaTSXYLF.js";
import "./hmac-C6LUpY3l.js";
import { n as sha256 } from "./ripemd160-CjKhWecw.js";
import "./utils-DTJIMQ4r.js";
import "./secp256k1-BcZDTLZ3.js";
import { n as import_eventemitter3 } from "./eventemitter3-Dkd_1R2c.js";
import { a as _, i as B, n as h, r as y, t as T } from "./hooks.module-B4rj-foG.js";
import { i as get$1, n as createStore, o as set, r as del } from "./dist-BAiCoMBQ.js";
import { C as baseSepolia, E as avalanche, O as arbitrum, b as bsc, c as polygon, d as optimism, f as mainnet, i as sepolia, n as zora, t as chains_exports, u as optimismSepolia, w as base$1 } from "./chains-c39jFqiu.js";
import { a as getUserOperationHash, c as u, i as toSmartAccount, n as secp256k1, o as entryPoint06Address, r as createBundlerClient, s as entryPoint06Abi, t as p256 } from "./p256-BgdTEi1k.js";
...
//#endregion
//#region node_modules/@base-org/account/node_modules/zustand/esm/middleware.mjs
...
//#endregion
//#region node_modules/@base-org/account/node_modules/zustand/esm/vanilla.mjs
...
const sdkstore = createStore$1(persist((...args) => ({
...
function logEvent(name$1, event, importance) {
	if (typeof window !== "undefined" && window.ClientAnalytics) window.ClientAnalytics?.logEvent(name$1, {
		...event,
...
function openPopup(url) {
	...
	function tryOpenPopup() {
		...
		const popup$1 = window.open(url, popupId, `width=${POPUP_WIDTH}, height=${POPUP_HEIGHT}, left=${left}, top=${top}`);
		popup$1?.focus();
...
const logHandshakeStarted = ({ method, correlationId }) => {
	const config$1 = store.subAccountsConfig.get();
	logEvent("scw_signer.handshake.started", {
		action: ActionType.unknown,
		componentType: ComponentType.unknown,
		method,
		correlationId,
		subAccountCreation: config$1?.creation,
		subAccountDefaultAccount: config$1?.defaultAccount,
		subAccountFunding: config$1?.funding
	}, AnalyticsEventImportance.high);
};
...
function defineChainConfig(chainId, rpcUrl, options) {
	...
	const name$1 = nativeCurrency?.name ?? viemChain?.name ?? "";
	return defineChain({
		id: chainId,
		name: name$1,
		nativeCurrency: {
...
async function encryptContent(content, sharedSecret) {
	return encrypt(sharedSecret, JSON.stringify(content, (_$1, value) => {
...
function sha256$1(value, options = {}) {
	...
	const bytes = sha256(from$1(value));
...
function toHex$2(publicKey, options = {}) {
	...
	return concat$1(includePrefix ? fromNumber(prefix, { size: 1 }) : "0x", fromNumber(x, { size: 32 }), typeof y$1 === "bigint" ? fromNumber(y$1, { size: 32 }) : "0x");
}
/**
...
//#endregion
//#region node_modules/@base-org/account/node_modules/ox/_esm/core/Solidity.js
...
function toHex$1(signature) {
	...
	const s = signature.s;
	return concat$1(fromNumber(r$1, { size: 32 }), fromNumber(s, { size: 32 }), typeof signature.yParity === "number" ? fromNumber(yParityToV(signature.yParity), { size: 1 }) : "0x");
}
/**
...
function toDerBytes(signature) {
	return new secp256k1.Signature(signature.r, signature.s).toDERRawBytes();
}
/**
...
function fromBytes(value, options = {}) {
	...
	for (let i = 0, j = 0; j < value.length; i += 4, j += 3) {
		...
		encoded[i] = integerToCharacter[y$1 >> 18];
		encoded[i + 1] = integerToCharacter[y$1 >> 12 & 63];
		encoded[i + 2] = integerToCharacter[y$1 >> 6 & 63];
		encoded[i + 3] = integerToCharacter[y$1 & 63];
	}
...
//#endregion
//#region node_modules/@base-org/account/node_modules/ox/_esm/core/WebAuthnP256.js
...
function getAuthenticatorData(options = {}) {
	...
	return concat$1(rpIdHash, flag_bytes, signCount_bytes);
}
/**
...
function getSignPayload(options) {
	...
	return {
		...
		payload: concat$1(authenticatorData, clientDataJSONHash)
	};
}
...
async function initSubAccountConfig() {
	const config$1 = store.subAccountsConfig.get() ?? {};
	const capabilities = {};
	if (config$1.creation === "on-connect") {
...
function createWalletSendCallsRequest({ calls, from: from$3, chainId, capabilities }) {
...
function validateDataSuffix$1(dataSuffix) {
	if (!isHex(dataSuffix)) throw new Error("Invalid dataSuffix: expected a 0x-prefixed hex string");
	return dataSuffix;
}
function makeDataSuffix({ attribution, dappOrigin }) {
	...
	if ("dataSuffix" in attribution && typeof attribution.dataSuffix === "string") return validateDataSuffix$1(attribution.dataSuffix);
}
/**
...
async function createSmartAccount(parameters) {
	...
	return toSmartAccount({
		...
		async signTypedData(parameters$1) {
			const { domain, types, primaryType, message } = parameters$1;
			return wrapSignature({
				ownerIndex,
				signature: await sign({
					hash: toReplaySafeHash({
		...
		async signUserOperation(parameters$1) {
			const { chainId = client.chain.id, ...userOperation } = parameters$1;
...
function validateGasLimitOverrides(calls) {
	for (let i = 0; i < calls.length; i++) {
		...
		if (gasLimitOverride) {
			...
			if (!value || !isHex(value)) throw standardErrors.rpc.invalidParams(`gasLimitOverride.value must be a hex string at call index ${i}`);
			if (hexToBigInt(value) === 0n) throw standardErrors.rpc.invalidParams(`gasLimitOverride.value cannot be zero at call index ${i}`);
		}
	}
}
...
async function handleAddSubAccountOwner({ ownerAccount, globalAccountRequest, chainId }) {
	...
	const globalAccount = account$1.accounts?.find((account$2) => account$2.toLowerCase() !== subAccount?.address.toLowerCase());
...
async function routeThroughGlobalAccount({ request, globalAccountAddress, subAccountAddress, client, globalAccountRequest, chainId, prependCalls }) {
	...
	const calls = [...prependCalls ?? [], batchCall];
...
async function aggregateGasLimitOverrides({ calls, client, subAccountAddress }) {
	if (!calls.some((call) => call.capabilities && "gasLimitOverride" in call.capabilities && call.capabilities.gasLimitOverride?.value)) return;
	const totalGas = (await Promise.all(calls.map(async (call) => {
		const override = call.capabilities?.gasLimitOverride?.value;
		if (override) return hexToBigInt(override);
	...
	const inputDataOverhead = calls.reduce((sum, call) => {
		return sum + (call.data ? BigInt((call.data.length - 2) / 2) : 0n) * PROPORTIONAL_INPUT_COST_PER_BYTE;
	}, 0n);
	return { gasLimitOverride: { value: numberToHex(totalGas + (BigInt(calls.length) * SAFETY_BUFFER_PER_CALL + inputDataOverhead)) } };
}
...
var Signer = class {
	...
	constructor(params) {
		...
		this.accounts = account$1.accounts ?? [];
		this.chain = account$1.chain ?? { id: params.metadata.appChainIds?.[0] ?? 1 };
	...
	async _request(request) {
		...
		if (request.method.startsWith("experimental_")) return this.sendRequestToPopup(request);
		switch (request.method) {
			case "eth_requestAccounts":
			case "eth_accounts": {
				const subAccount = store.subAccounts.get();
				const subAccountsConfig$1 = store.subAccountsConfig.get();
				if (subAccount?.address) this.accounts = subAccountsConfig$1?.defaultAccount === "sub" ? prependWithoutDuplicates(this.accounts, subAccount.address) : appendWithoutDuplicates(this.accounts, subAccount.address);
	...
	async handleResponse(request, decrypted) {
		...
		switch (request.method) {
			...
			case "wallet_connect": {
				...
				const accounts = response.accounts.map((account$1) => account$1.address);
				this.accounts = accounts;
				...
				const subAccountsConfig$1 = store.subAccountsConfig.get();
				if (subAccount?.address) this.accounts = subAccountsConfig$1?.defaultAccount === "sub" ? prependWithoutDuplicates(this.accounts, subAccount.address) : appendWithoutDuplicates(this.accounts, subAccount.address);
				const spendPermissions$1 = response?.accounts?.[0].capabilities?.spendPermissions;
				if (spendPermissions$1 && "permissions" in spendPermissions$1) store.spendPermissions.set(spendPermissions$1?.permissions);
	...
	*/
	...
	async handleGetCapabilitiesRequest(request) {
		...
		const sdkWildcardCapabilities = { gasLimitOverride: { supported: true } };
	...
	updateChain(chainId, newAvailableChains) {
		...
		const chain = (newAvailableChains ?? state.chains)?.find((chain$1) => chain$1.id === chainId);
	...
	async addSubAccount(request) {
		const cachedSubAccount = store.getState().subAccount;
		const subAccountsConfig$1 = store.subAccountsConfig.get();
		...
		if (cachedSubAccount?.address) {
			if (!requestedAddress || isAddressEqual(requestedAddress, cachedSubAccount.address)) {
				this.accounts = subAccountsConfig$1?.defaultAccount === "sub" ? prependWithoutDuplicates(this.accounts, cachedSubAccount.address) : appendWithoutDuplicates(this.accounts, cachedSubAccount.address);
				this.callback?.("accountsChanged", this.accounts);
	...
	async sendRequestToSubAccountSigner(request) {
		...
		const ownerAccount = subAccountsConfig$1?.toOwnerAccount ? await subAccountsConfig$1.toOwnerAccount() : await getCryptoKeyAccount();
		...
		const dataSuffix = makeDataSuffix({
			attribution: config$1.preference?.attribution,
...
function getInjectedProvider() {
	try {
		...
		logGetInjectedProviderError({ errorMessage: parseErrorMessageFromAny(error) });
		return null;
	}
}
	...
	const sdk = {
		...
		subAccount: {
			...
			async get() {
				...
				const subAccounts$1 = (await sdk.getProvider()?.request({
					method: "wallet_connect",
					params: [{
						version: "1",
						capabilities: {}
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/payment/constants.js
/**
...
//#endregion
//#region node_modules/@base-org/account/dist/core/telemetry/events/payment.js
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/payment/getPaymentStatus.js
/**
...
async function getPaymentStatus(options) {
	...
	try {
		...
		if (success) {
			...
			if (txReceipt?.logs) {
				...
				for (let i = 0; i < txReceipt.logs.length; i++) {
					...
					if (log.address?.toLowerCase() === usdcAddress) try {
				...
				if (usdcTransfers.length > 0 && senderAddress) {
					...
					if (senderTransfers.length === 0) throw new Error(`Unable to find USDC transfer from sender wallet ${receipt.result.sender}. Found ${usdcTransfers.length} USDC transfer(s) but none originated from the sender wallet.`);
		...
		if (reason) if (reason.toLowerCase().includes("insufficient")) userFriendlyReason = "Insufficient USDC balance";
		else userFriendlyReason = reason;
		if (telemetry) logPaymentStatusCheckCompleted({
			testnet,
			status: "failed",
			correlationId
...
//#endregion
//#region node_modules/@base-org/account/dist/core/telemetry/events/spend-permission.js
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/spend-permission/withTelemetry.js
function withTelemetry(fn) {
	if (store.config.get().preference?.telemetry === false) return fn;
	return (...args) => {
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/spend-permission/methods/fetchPermission.js
/**
...
var fetchPermissionFn = async ({ provider, permissionHash }) => {
	let response;
	if (provider) response = await provider.request({
		method: "coinbase_fetchPermission",
		params: [{ permissionHash }]
	});
	else response = await fetchRPCRequest({
		method: "coinbase_fetchPermission",
		params: [{ permissionHash }]
	}, CB_WALLET_RPC_URL);
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/spend-permission/methods/getHash.js
/**
...
var getHashFn = async ({ permission, chainId }) => {
	const client = getClient(chainId);
	if (!client) throw new Error(`No client found for chain ID ${chainId}. Chain not supported or RPC URL not available`);
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/multicall/index.js
/**
...
* @public
*/
async function multicall(client, config$1) {
	const { contracts, allowPartialFailure = false, errorMessages = [] } = config$1;
	if (!client) throw new Error("Public client is required for multicall");
	if (!contracts || contracts.length === 0) throw new Error("At least one contract call is required");
	try {
		return (await multicall$1(client, { contracts })).map((result, index) => {
			if (result.status === "success") return {
				status: "success",
				result: result.result
			};
			const failureError = result.status === "failure" && "error" in result ? result.error : /* @__PURE__ */ new Error("Unknown error");
			const error = new Error(errorMessages[index] || `Contract call ${index} failed: ${failureError.message || "Unknown error"}`);
			if (!allowPartialFailure) throw error;
			return {
				status: "failure",
				error
			};
		...
		if (error instanceof Error && (error.message.includes("Contract call") || error.message.includes("required"))) throw error;
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/spend-permission/utils.js
var ETERNITY_TIMESTAMP = 0xffffffffffff;
var SPEND_PERMISSION_TYPED_DATA_TYPES = { SpendPermission: [
	{
		name: "account",
		type: "address"
	},
...
function createSpendPermissionTypedData(request) {
	const { account: account$1, spender, token, chainId, allowance, periodInDays, start, end, salt, extraData } = request;
	return {
		domain: {
...
function createSpendPermissionTypedDataWithSeconds(request) {
	const { account: account$1, spender, token, chainId, allowance, periodInSeconds, start, end, salt, extraData } = request;
	return {
		domain: {
			name: "Spend Permission Manager",
			version: "1",
...
function toSpendPermissionArgs(permission) {
	const { account: account$1, spender, token, allowance: allowanceStr, period, start, end, salt, extraData } = permission.permission;
	return {
		account: getAddress(account$1),
		spender: getAddress(spender),
		token: getAddress(token),
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/spend-permission/utils.node.js
/**
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/spend-permission/methods/getPermissionStatus.js
/**
...
var getPermissionStatusFn = async (permission, options) => {
	...
	if (!chainId) throw new Error("chainId is missing in the spend permission");
	let client;
	if (rpcUrl) {
		const chain = getPublicClientFromChainId(chainId)?.chain;
		...
		if (!client) client = getPublicClientFromChainId(chainId);
	}
	if (!client) throw new Error(`No client available for chain ID ${chainId}. Chain is not supported.`);
	const spendPermissionArgs = toSpendPermissionArgs(permission);
	const [currentPeriod, isRevoked, isValid] = unwrapMulticallResults(await multicall(client, {
		contracts: [
			{
				address: spendPermissionManagerAddress,
				abi: spendPermissionManagerAbi,
	...
	const isExpired = Math.floor(Date.now() / 1e3) > permission.permission.end;
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/spend-permission/methods/prepareRevokeCallData.js
/**
...
var prepareRevokeCallDataFn = async (permission) => {
	return {
		...
		data: encodeFunctionData({
			...
			args: [toSpendPermissionArgs(permission)]
		}),
		value: 0n
	};
};
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/spend-permission/methods/prepareSpendCallData.js
/**
...
var prepareSpendCallDataFn = async (permission, amount, recipient, options) => {
	...
	if (isRevoked) throw new Error("Spend permission has been revoked");
	const spendAmount = amount === "max-remaining-allowance" ? remainingSpend : amount;
	if (spendAmount === BigInt(0)) throw new Error("Spend amount cannot be 0");
	if (spendAmount > remainingSpend) throw new Error("Remaining spend amount is insufficient");
	let approveCall = null;
	const spendPermissionArgs = toSpendPermissionArgs(permission);
	if (!isApprovedOnchain) approveCall = {
		to: spendPermissionManagerAddress,
		data: encodeFunctionData({
			abi: spendPermissionManagerAbi,
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/payment/getSubscriptionStatus.js
/**
...
async function getSubscriptionStatus(options) {
	...
	if (permission.chainId !== expectedChainId) {
		...
		let errorMessage;
		if (testnet && isSubscriptionOnMainnet) errorMessage = "The subscription was requested on testnet but is actually a mainnet subscription";
		else if (!testnet && isSubscriptionOnTestnet) errorMessage = "The subscription was requested on mainnet but is actually a testnet subscription";
		else errorMessage = `Subscription is on chain ${permission.chainId}, expected ${expectedChainId} (${testnet ? "Base Sepolia" : "Base"})`;
		throw new Error(errorMessage);
	}
	if (permission.permission.token.toLowerCase() !== expectedTokenAddress) throw new Error(`Subscription is not for USDC token. Got ${permission.permission.token}, expected ${expectedTokenAddress}`);
	...
	if (currentTime < permissionStart) throw new Error(`Subscription has not started yet. It will begin at ${(/* @__PURE__ */ new Date(permissionStart * 1e3)).toISOString()}`);
	return {
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/payment/utils/sdkManager.js
/**
...
function createEphemeralSDK(chainId, walletUrl, telemetry = true, dataSuffix) {
	return createBaseAccountSDK({
		appName: typeof window !== "undefined" ? window.location.origin : "Base Pay SDK",
		appChainIds: [chainId],
		preference: {
			telemetry,
			walletUrl,
...
async function executePayment(sdk, requestParams) {
	...
	if (typeof result === "string" && result.length >= 66) transactionHash = result.slice(0, 66);
		...
		if (typeof resultObj.id === "string" && resultObj.id.length >= 66) {
			...
			if (resultObj.capabilities?.dataCallback) payerInfoResponses = resultObj.capabilities.dataCallback;
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/payment/utils/translatePayment.js
/**
...
function buildSendCallsRequest(transferData, testnet, payerInfo, dataSuffix) {
	...
	if (dataSuffix) capabilities.attribution = { suffix: dataSuffix };
	return {
		version: "2.0.0",
		chainId,
		calls: [call],
...
function translatePaymentToSendCalls(recipient, amount, testnet, payerInfo, dataSuffix) {
	return buildSendCallsRequest(encodeTransferCall(recipient, amount), testnet, payerInfo, dataSuffix);
}

//#endregion
//#region node_modules/@base-org/account/dist/interface/payment/utils/validation.js
/**
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/payment/pay.js
/**
...
async function pay(options) {
	...
	try {
		...
		if (dataSuffix !== void 0) validateDataSuffix(dataSuffix);
		const executionResult = await executePaymentWithSDK(translatePaymentToSendCalls(normalizedAddress, amount, testnet, payerInfo, dataSuffix), testnet, walletUrl, telemetry, dataSuffix);
		...
		if (error instanceof Error) errorMessage = error.message;
			...
			else if (typeof err?.error?.message === "string") errorMessage = err.error.message;
			else if (typeof err?.reason === "string") errorMessage = err.reason;
		}
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/payment/utils/validateUSDCBasePermission.js
/**
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/payment/prepareCharge.js
/**
...
async function prepareCharge(options) {
	...
	const permission = await fetchPermission({ permissionHash: id });
	if (!permission) throw new Error(`Subscription with ID ${id} not found`);
	validateUSDCBasePermission(permission, testnet);
	let spendAmount;
	if (amount === "max-remaining-charge") spendAmount = "max-remaining-allowance";
	else spendAmount = parseUnits(amount, TOKENS.USDC.decimals);
	return await prepareSpendCallData(permission, spendAmount, recipient, { rpcUrl });
}

//#endregion
//#region node_modules/@base-org/account/dist/interface/payment/prepareRevoke.js
/**
...
async function prepareRevoke(options) {
	...
	if (!permission) throw new Error(`Subscription with ID ${id} not found`);
	validateUSDCBasePermission(permission, testnet);
	return await prepareRevokeCallData(permission);
}

//#endregion
//#region node_modules/@base-org/account/dist/core/telemetry/events/subscription.js
/**
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/payment/subscribe.js
...
async function subscribe(options) {
	...
	if (hasOverridePeriod && !testnet) throw new Error("overridePeriodInSecondsForTestnet is only available for testing on testnet. Set testnet: true to use overridePeriodInSecondsForTestnet, or use periodInDays for production.");
	const overridePeriodInSecondsForTestnet = testnet && hasOverridePeriod ? options.overridePeriodInSecondsForTestnet : void 0;
	const correlationId = crypto.randomUUID();
	if (telemetry) logSubscriptionStarted({
		recurringCharge,
	...
	try {
		...
		const provider = createEphemeralSDK(chainId, walletUrl, telemetry).getProvider();
		try {
			...
			if (telemetry) logSubscriptionCompleted({
				recurringCharge,
				periodInDays: testnet && overridePeriodInSecondsForTestnet !== void 0 ? Math.ceil(overridePeriodInSecondsForTestnet / 86400) : periodInDays,
				periodInSeconds: testnet ? overridePeriodInSecondsForTestnet : void 0,
				testnet,
			...
			return {
				...
				recurringCharge,
				periodInDays: testnet && overridePeriodInSecondsForTestnet !== void 0 ? Math.ceil(overridePeriodInSecondsForTestnet / 86400) : periodInDays,
				...testnet && overridePeriodInSecondsForTestnet !== void 0 && { overridePeriodInSecondsForTestnet }
			};
		} finally {
			await provider.disconnect();
		}
		...
		if (telemetry) logSubscriptionError({
			recurringCharge,
			periodInDays: testnet && overridePeriodInSecondsForTestnet !== void 0 ? Math.ceil(overridePeriodInSecondsForTestnet / 86400) : periodInDays,
			periodInSeconds: testnet ? overridePeriodInSecondsForTestnet : void 0,
			testnet,
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/payment/base.browser.js
/**
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/prolink/shortcuts/generic.js
/**
...
function encodeGenericRpc(method, params) {
	...
	return {
		method,
		paramsJson: new TextEncoder().encode(paramsJson),
		rpcVersion: "2.0"
	};
}
/**
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/prolink/types.js
/**
...
*/
var SendCallsType;
(function(SendCallsType$1) {
	SendCallsType$1[SendCallsType$1["SEND_CALLS_UNKNOWN"] = 0] = "SEND_CALLS_UNKNOWN";
	SendCallsType$1[SendCallsType$1["ERC20_TRANSFER"] = 1] = "ERC20_TRANSFER";
	SendCallsType$1[SendCallsType$1["NATIVE_TRANSFER"] = 2] = "NATIVE_TRANSFER";
	SendCallsType$1[SendCallsType$1["GENERIC_CALLS"] = 3] = "GENERIC_CALLS";
...
var SignType;
(function(SignType$1) {
	SignType$1[SignType$1["SIGN_UNKNOWN"] = 0] = "SIGN_UNKNOWN";
	SignType$1[SignType$1["SPEND_PERMISSION"] = 1] = "SPEND_PERMISSION";
	SignType$1[SignType$1["RECEIVE_WITH_AUTHORIZATION"] = 2] = "RECEIVE_WITH_AUTHORIZATION";
	SignType$1[SignType$1["GENERIC_TYPED_DATA"] = 3] = "GENERIC_TYPED_DATA";
})(SignType || (SignType = {}));

//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/prolink/utils/encoding.js
/**
...
function encodeAddress(address) {
	...
	if (normalized.length !== 42) throw new Error(`Invalid address length: expected 40 hex chars, got ${normalized.length - 2}`);
	const bytes = fromHex$3(normalized);
	if (bytes.length !== 20) throw new Error(`Invalid address length: expected 20 bytes, got ${bytes.length}`);
	return bytes;
}
/**
...
function encodeAmount(value) {
	...
	if (typeof value === "string") {
		...
		if (normalized === "" || normalized === "0") return new Uint8Array([0]);
		bigintValue = BigInt(`0x${normalized}`);
	} else bigintValue = value;
	if (bigintValue === 0n) return new Uint8Array([0]);
	if (bigintValue < 0n) throw new Error("Cannot encode negative amounts");
	const hex = bigintValue.toString(16);
	const paddedHex = hex.length % 2 === 0 ? hex : `0${hex}`;
	return fromHex$3(`0x${paddedHex}`);
}
/**
...
function decodeCapabilities(map) {
	const caps = {};
	for (const [key, bytes] of map.entries()) try {
...
function pad32(bytes) {
	if (bytes.length > 32) throw new Error(`Cannot pad value larger than 32 bytes: ${bytes.length}`);
...
function bytesToHex(bytes) {
	...
	let hex = fromBytes$2(bytes).replace(/^0x0+/, "0x") || "0x0";
	if (hex === "0x") hex = "0x0";
	return hex;
}
/**
...
function hexToBytes(hex) {
	...
	return fromHex$3(normalized);
}

//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/prolink/shortcuts/sendCalls.js
...
function detectErc20Transfer(calls) {
	...
	if (dataNoPrefix.length !== 136) return null;
	const recipient = `0x${dataNoPrefix.slice(8, 72).slice(24)}`;
...
function encodeWalletSendCalls(params) {
	...
	return {
		...
		transactionData: {
			case: "genericCalls",
			value: { calls: params.calls.map((call) => ({
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/prolink/shortcuts/sign.js
/**
...
function encodeWalletSign(params) {
	...
	if (normalizedType !== "0x01") throw new Error(`Unsupported sign type for prolink encoding: ${normalizedType}`);
	...
	if (paramsChainId !== domainChainId) throw new Error(`Chain ID mismatch: params has ${paramsChainId}, domain has ${domainChainId}`);
		...
		return {
			...
			signatureData: {
				...
				value: {
					...
					extraData: !msg.extraData || msg.extraData === "0x" ? new Uint8Array() : hexToBytes(msg.extraData),
	...
	return {
		...
		signatureData: {
			...
			value: { typedDataJson: typedDataBytes }
		},
		version: params.version || "1"
	};
}
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/prolink/utils/base64url.js
/**
...
function decodeBase64url(payload) {
	for (let i = 0; i < payload.length; i++) {
		...
		if (!BASE64URL_ALPHABET.includes(char) && char !== "=") throw new Error(`Invalid Base64url character at position ${i}: '${char}'. Only A-Z, a-z, 0-9, -, _ are allowed.`);
	}
	const paddingIndex = payload.indexOf("=");
	if (paddingIndex !== -1) {
		const paddingPart = payload.slice(paddingIndex);
		if (paddingPart !== "=" && paddingPart !== "==") throw new Error("Invalid Base64url padding");
		for (let i = paddingIndex; i < payload.length; i++) if (payload[i] !== "=") throw new Error("Invalid Base64url: characters found after padding");
	}
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/prolink/utils/compression.js
/**
...
async function ensureBrotliInitialized() {
	if (!brotliModule) try {
		brotliModule = await (await import("./index.web-Cj1ke-tC.js")).default;
	} catch (error) {
...
async function compressPayload(data) {
	...
	if (!brotliModule) throw new Error("Brotli module not initialized");
	const compressed = brotliModule.compress(data, {
		quality: 5,
		lgwin: 22
	});
	if (compressed.length + 1 < data.length + 1) return {
		compressed,
...
async function decompressPayload(data) {
	if (data.length === 0) throw new Error("Cannot decompress empty payload");
	const flag = data[0];
	const payload = data.slice(1);
	if (flag === COMPRESSION_FLAG_NONE) return payload;
	if (flag === COMPRESSION_FLAG_BROTLI) {
		await ensureBrotliInitialized();
		if (!brotliModule) throw new Error("Brotli module not initialized");
		try {
			return brotliModule.decompress(payload);
		} catch (error) {
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/prolink/utils/protobuf.js
...
function encodeBytes(fieldNumber, value) {
	...
	const tagBytes = encodeVarint(fieldNumber << 3 | WIRE_TYPE_LENGTH_DELIMITED);
...
function encodeVarintField(fieldNumber, value) {
	...
	const tagBytes = encodeVarint(fieldNumber << 3 | WIRE_TYPE_VARINT);
...
*/
function encodeWalletSendCalls$1(value) {
	...
	if (value.from) parts.push(encodeBytes(3, value.from));
	if (value.version) parts.push(encodeString(4, value.version));
	return concat(...parts);
}
/**
...
*/
function encodeWalletSign$1(value) {
	const parts = [encodeVarintField(1, value.type)];
	...
	if (value.version) parts.push(encodeString(3, value.version));
	return concat(...parts);
}
/**
...
function encodeRpcLinkPayload(value) {
	...
	if (value.body.case === "generic") {
		...
		const encoded = encodeWalletSendCalls$1(value.body.value);
		parts.push(encodeMessage(11, encoded));
	} else if (value.body.case === "walletSign") {
		const encoded = encodeWalletSign$1(value.body.value);
		parts.push(encodeMessage(12, encoded));
	}
	if (value.capabilities && value.capabilities.size > 0) parts.push(encodeMap(20, value.capabilities));
	return concat(...parts);
}
/**
...
function decodeRpcLinkPayload(buffer) {
	...
	if (fields.has(10)) body = {
		case: "generic",
		value: decodeGenericJsonRpc(fields.get(10))
	};
	else if (fields.has(11)) body = {
		case: "walletSendCalls",
		value: decodeWalletSendCalls$1(fields.get(11))
	};
	else if (fields.has(12)) body = {
		case: "walletSign",
		value: decodeWalletSign$1(fields.get(12))
	};
...
function parseMapEntry(buffer) {
	...
	if (!keyBytes || !valueBytes) throw new Error("Invalid map entry: missing key or value");
	return {
...
*/
function decodeWalletSendCalls$1(buffer) {
	const fields = parseFields(buffer);
	const type = Number(fields.get(1) || 0n);
	const fromBytes$3 = fields.get(3);
	const versionBytes = fields.get(4);
	let transactionData = { case: void 0 };
	if (fields.has(10)) transactionData = {
		case: "erc20Transfer",
		value: decodeErc20Transfer(fields.get(10))
	};
	else if (fields.has(11)) transactionData = {
		case: "nativeTransfer",
		value: decodeNativeTransfer(fields.get(11))
	};
	else if (fields.has(12)) transactionData = {
		case: "genericCalls",
		value: decodeGenericCalls(fields.get(12))
	};
	return {
		type,
		transactionData,
...
function decodeGenericCalls(buffer) {
	...
	while (offset < buffer.length) {
		...
		} else throw new Error(`Unexpected field in GenericCalls: ${fieldNumber}`);
	}
	return { calls };
}
/**
...
*/
function decodeWalletSign$1(buffer) {
	const fields = parseFields(buffer);
	const type = Number(fields.get(1) || 0n);
	const versionBytes = fields.get(3);
	let signatureData = { case: void 0 };
	if (fields.has(10)) signatureData = {
		case: "spendPermission",
		value: decodeSpendPermission(fields.get(10))
	};
	else if (fields.has(11)) signatureData = {
		case: "receiveWithAuthorization",
		value: decodeReceiveWithAuthorization(fields.get(11))
	};
	else if (fields.has(12)) signatureData = {
		case: "genericTypedData",
		value: decodeGenericTypedData(fields.get(12))
	};
	return {
		type,
		signatureData,
...
function decodeGenericTypedData(buffer) {
	return { typedDataJson: parseFields(buffer).get(1) || new Uint8Array() };
}

//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/prolink/createProlinkUrl.js
/**
...
function createProlinkUrl(prolink, url = "https://base.app/base-pay", additionalQueryParams) {
	if (!prolink || prolink.trim().length === 0) throw new Error("prolink cannot be empty");
	if (!url || url.trim().length === 0) throw new Error("url cannot be empty");
...
//#endregion
//#region node_modules/@base-org/account/dist/interface/public-utilities/prolink/index.js
/**
...
async function encodeProlink(request) {
	...
	if (request.method === "wallet_sendCalls") {
		if (!Array.isArray(request.params) || request.params.length === 0) throw new Error("wallet_sendCalls requires params array with at least one element");
		const params = request.params[0];
		if (typeof params !== "object" || !params) throw new Error("wallet_sendCalls params[0] must be an object");
		const chainIdHex = params.chainId;
		if (!chainIdHex) throw new Error("wallet_sendCalls requires chainId in params");
		payload = {
			...
			body: {
				case: "walletSendCalls",
				value: encodeWalletSendCalls(params)
			},
			capabilities: request.capabilities ? encodeCapabilities(request.capabilities) : void 0
		};
	} else if (request.method === "wallet_sign") {
		if (!Array.isArray(request.params) || request.params.length === 0) throw new Error("wallet_sign requires params array with at least one element");
		const params = request.params[0];
		if (typeof params !== "object" || !params) throw new Error("wallet_sign params[0] must be an object");
		const chainIdHex = params.chainId;
		if (!chainIdHex) throw new Error("wallet_sign requires chainId in params");
		payload = {
			protocolVersion: PROTOCOL_VERSION,
			chainId: Number.parseInt(chainIdHex, 16),
	...
	const { compressed, flag } = await compressPayload(encodeRpcLinkPayload(payload));
...
async function decodeProlink(payload) {
	const rpcPayload = decodeRpcLinkPayload(await decompressPayload(decodeBase64url(payload)));
	if (rpcPayload.protocolVersion !== PROTOCOL_VERSION) throw new Error(`Unsupported protocol version: ${rpcPayload.protocolVersion} (expected ${PROTOCOL_VERSION})`);
	...
	if (rpcPayload.shortcutId === SHORTCUT_GENERIC) {
		if (rpcPayload.body.case !== "generic") throw new Error("Invalid payload: shortcut 0 requires generic body");
	...
	if (rpcPayload.shortcutId === SHORTCUT_WALLET_SEND_CALLS) {
		if (rpcPayload.body.case !== "walletSendCalls") throw new Error("Invalid payload: shortcut 1 requires walletSendCalls body");
		if (!rpcPayload.chainId) throw new Error("wallet_sendCalls requires chainId");
		return {
			method: "wallet_sendCalls",
			params: [decodeWalletSendCalls(rpcPayload.body.value, rpcPayload.chainId, capabilities)]
		};
	}
	if (rpcPayload.shortcutId === SHORTCUT_WALLET_SIGN) {
		if (rpcPayload.body.case !== "walletSign") throw new Error("Invalid payload: shortcut 2 requires walletSign body");
		if (!rpcPayload.chainId) throw new Error("wallet_sign requires chainId");
		return {
			method: "wallet_sign",
			params: [decodeWalletSign(rpcPayload.body.value, rpcPayload.chainId, capabilities)]
		};
	}
...
//#endregion
export { CHAIN_IDS, TOKENS, PACKAGE_VERSION as VERSION, base, createBaseAccountSDK, createProlinkUrl, decodeProlink, encodeProlink, getCryptoKeyAccount, getPaymentStatus, getSubscriptionStatus, pay, prepareCharge, removeCryptoKey, subscribe };
//# sourceMappingURL=dist-CqyCIFUU.js.map
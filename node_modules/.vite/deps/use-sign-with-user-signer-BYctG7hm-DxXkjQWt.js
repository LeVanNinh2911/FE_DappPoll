...
import { pn as I } from "./useActiveWallet-9-7-ELBX-C7cHxRLi.js";
...
function isSolanaError(e$2, code) {
	if (e$2 instanceof Error && e$2.name === "SolanaError") {
		if (code !== void 0) return e$2.context.__code === code;
		return true;
	}
	return false;
}
...
function getSolanaErrorFromInstructionError(index, instructionError) {
	...
	return getSolanaErrorFromRpcError({
		...
		getErrorContext(errorCode, rpcErrorName, rpcErrorContext) {
			if (errorCode === SOLANA_ERROR__INSTRUCTION_ERROR__UNKNOWN) return {
				errorName: rpcErrorName,
				index: numberIndex,
				...rpcErrorContext !== void 0 ? { instructionErrorContext: rpcErrorContext } : null
			};
			else if (errorCode === SOLANA_ERROR__INSTRUCTION_ERROR__CUSTOM) return {
...
function getSolanaErrorFromTransactionError(transactionError) {
	...
	return getSolanaErrorFromRpcError({
		...
		getErrorContext(errorCode, rpcErrorName, rpcErrorContext) {
			if (errorCode === SOLANA_ERROR__TRANSACTION_ERROR__UNKNOWN) return {
				errorName: rpcErrorName,
				...rpcErrorContext !== void 0 ? { transactionErrorContext: rpcErrorContext } : null
			};
			else if (errorCode === SOLANA_ERROR__TRANSACTION_ERROR__DUPLICATE_INSTRUCTION) return { index: Number(rpcErrorContext) };
			else if (errorCode === SOLANA_ERROR__TRANSACTION_ERROR__INSUFFICIENT_FUNDS_FOR_RENT || errorCode === SOLANA_ERROR__TRANSACTION_ERROR__PROGRAM_EXECUTION_TEMPORARILY_RESTRICTED) return { accountIndex: Number(rpcErrorContext.account_index) };
		},
...
function getSolanaErrorFromJsonRpcError(putativeErrorResponse) {
	...
	} else out = new SolanaError(SOLANA_ERROR__MALFORMED_JSON_RPC_ERROR, {
...
function containsBytes(data, bytes, offset) {
	return bytesEqual(offset === 0 && data.length === bytes.length ? data : data.slice(offset, offset + bytes.length), bytes);
}
...
function assertValidBaseString$1(alphabet4, testValue, givenValue = testValue) {
	if (!testValue.match(/* @__PURE__ */ new RegExp(`^[${alphabet4}]*$`))) throw new SolanaError(SOLANA_ERROR__CODECS__INVALID_STRING_FOR_BASE, {
		alphabet: alphabet4,
...
function partitionLeadingZeroes$1(value, zeroCharacter) {
	const [leadingZeros, tailChars] = value.split(/* @__PURE__ */ new RegExp(`((?!${zeroCharacter}).*)`));
...
var getBase58Decoder = () => getBaseXDecoder$1(alphabet2$1);
...
function parseBase64RpcAccount(address$1, rpcAccount) {
...
function parseJsonRpcAccount(address$1, rpcAccount) {
	if (!rpcAccount) return Object.freeze({
		address: address$1,
		exists: false
	});
...
async function fetchEncodedAccount(rpc, address$1, config = {}) {
	const { abortSignal, ...rpcConfig } = config;
	return parseBase64RpcAccount(address$1, (await rpc.getAccountInfo(address$1, {
		...rpcConfig,
		encoding: "base64"
...
async function getProgramDerivedAddress({ programAddress, seeds }) {
	...
	while (bumpSeed > 0) try {
		...
		if (isSolanaError(e$2, SOLANA_ERROR__ADDRESSES__INVALID_SEEDS_POINT_ON_CURVE)) bumpSeed--;
		else throw e$2;
	}
...
function getArrayEncoder(item, config = {}) {
	...
	return createEncoder({
		...
		write: (array, bytes, offset) => {
			if (typeof size === "number") assertValidNumberOfItemsForCodec("array", size, array.length);
...
var getBase58Decoder$1 = () => getBaseXDecoder(alphabet2);
...
function getTransactionVersionEncoder() {
	return createEncoder({
		...
		write: (value, bytes, offset) => {
			...
			if (value > MAX_SUPPORTED_TRANSACTION_VERSION) throw new SolanaError(SOLANA_ERROR__TRANSACTION__VERSION_NUMBER_NOT_SUPPORTED, { unsupportedVersion: value });
	...
	return createDecoder({
		...
		read: (bytes, offset) => {
			...
			if ((firstByte & VERSION_FLAG_MASK) === 0) return ["legacy", offset];
				...
				if (version > MAX_SUPPORTED_TRANSACTION_VERSION) throw new SolanaError(SOLANA_ERROR__TRANSACTION__VERSION_NUMBER_NOT_SUPPORTED, { unsupportedVersion: version });
...
function getCompiledMessageVersionedEncoder() {
	return transformEncoder(getStructEncoder([...getPreludeStructEncoderTuple(), ["addressTableLookups", getAddressTableLookupArrayEncoder()]]), (value) => {
...
function getPreludeStructEncoderTuple() {
	const lifetimeTokenEncoder = getUnionEncoder([getConstantEncoder(new Uint8Array(32)), fixEncoderSize(getBase58Encoder(), 32)], (value) => value === void 0 ? 0 : 1);
	return [
...
function getPreludeStructDecoderTuple() {
	return [
		...
		["lifetimeToken", fixDecoderSize(getBase58Decoder$1(), 32)],
...
function getCompiledTransactionMessageEncoder() {
	return createEncoder({
		getSizeFromValue: (compiledMessage) => {
			if (compiledMessage.version === "legacy") return getCompiledMessageLegacyEncoder().getSizeFromValue(compiledMessage);
			else return getCompiledMessageVersionedEncoder().getSizeFromValue(compiledMessage);
		},
		write: (compiledMessage, bytes, offset) => {
			if (compiledMessage.version === "legacy") return getCompiledMessageLegacyEncoder().write(compiledMessage, bytes, offset);
			else return getCompiledMessageVersionedEncoder().write(compiledMessage, bytes, offset);
		}
	});
}
...
function upsert(addressMap, address$1, update) {
	addressMap[address$1] = update(addressMap[address$1] ?? { role: AccountRole.READONLY });
}
...
function getOrderedAccountsFromAddressMap(addressMap) {
	...
	return Object.entries(addressMap).sort(([leftAddress, leftEntry], [rightAddress, rightEntry]) => {
		if (leftEntry[TYPE] !== rightEntry[TYPE]) {
			...
			else if (leftEntry[TYPE] === 2) return -1;
			else if (rightEntry[TYPE] === 2) return 1;
		}
		...
		if (leftEntry[TYPE] === 1 && rightEntry[TYPE] === 1 && leftEntry.lookupTableAddress !== rightEntry.lookupTableAddress) return addressComparator(leftEntry.lookupTableAddress, rightEntry.lookupTableAddress);
		else return addressComparator(leftAddress, rightAddress);
	}).map(([address$1, addressMeta]) => ({
		address: address$1,
		...addressMeta
	}));
}
...
function getCompiledInstructions(instructions, orderedAccounts) {
	...
	return instructions.map(({ accounts, data, programAddress }) => {
		return {
			...
			...accounts ? { accountIndices: accounts.map(({ address: address$1 }) => accountIndex[address$1]) } : null,
			...data ? { data } : null
		};
	});
}
...
function getCompiledStaticAccounts(orderedAccounts) {
	...
	return (firstLookupTableAccountIndex === -1 ? orderedAccounts : orderedAccounts.slice(0, firstLookupTableAccountIndex)).map(({ address: address$1 }) => address$1);
}
...
function getAddressLookupMetas(compiledAddressTableLookups, addressesByLookupTableAddress) {
	...
	for (const lookup of compiledAddressTableLookups) {
		...
		const readOnlyForLookup = readonlyIndexes.map((r$1) => ({
			address: addresses[r$1],
			addressIndex: r$1,
			lookupTableAddress: lookup.lookupTableAddress,
			role: AccountRole.READONLY
		}));
...
function getTransactionEncoder() {
	return getStructEncoder([["signatures", getSignaturesEncoder()], ["messageBytes", getBytesEncoder()]]);
}
function getTransactionDecoder() {
	return transformDecoder(getStructDecoder([["signatures", getArrayDecoder(fixDecoderSize(getBytesDecoder(), 64), { size: getShortU16Decoder() })], ["messageBytes", getBytesDecoder()]]), decodePartiallyDecodedTransaction);
}
function decodePartiallyDecodedTransaction(transaction) {
	...
	const signaturesMap = {};
	signerAddresses.forEach((address$1, index) => {
		const signatureForAddress = signatures[index];
		if (signatureForAddress.every((b) => b === 0)) signaturesMap[address$1] = null;
		else signaturesMap[address$1] = signatureForAddress;
	});
	return {
		messageBytes,
...
async function partiallySignTransaction(keyPairs, transaction) {
	...
	await Promise.all(keyPairs.map(async (keyPair) => {
		const address$1 = await getAddressFromPublicKey(keyPair.publicKey);
		const existingSignature = transaction.signatures[address$1];
		if (existingSignature === void 0) {
			unexpectedSigners ||= /* @__PURE__ */ new Set();
			unexpectedSigners.add(address$1);
			return;
		}
		...
		newSignatures ||= {};
		newSignatures[address$1] = newSignature;
	}));
...
function getDefaultRequestTransformerForSolanaRpc(config) {
	...
	return (request) => {
		return pipe(request, handleIntegerOverflow ? getIntegerOverflowRequestTransformer(handleIntegerOverflow) : (r$1) => r$1, getBigIntDowncastRequestTransformer(), getDefaultCommitmentRequestTransformer({
...
function getDefaultResponseTransformerForSolanaRpc(config) {
	return (response, request) => {
		...
		return pipe(response, (r$1) => getThrowSolanaErrorResponseTransformer()(r$1, request), (r$1) => getResultResponseTransformer()(r$1, request), (r$1) => getBigIntUpcastResponseTransformer(keyPaths ?? [])(r$1, request));
	};
}
function getDefaultResponseTransformerForSolanaRpcSubscriptions(config) {
	return (response, request) => {
		const methodName = request.methodName;
		const keyPaths = config?.allowedNumericKeyPaths && methodName ? config.allowedNumericKeyPaths[methodName] : void 0;
		return pipe(response, (r$1) => getBigIntUpcastResponseTransformer(keyPaths ?? [])(r$1, request));
	};
}
			...
			coalescedRequestsByDeduplicationKey[deduplicationKey] = {
				...
				responsePromise: (async () => {
					try {
						...
						if (e$2 === (EXPLICIT_ABORT_TOKEN$1 ||= createExplicitAbortToken$1())) return;
						throw e$2;
					}
				})()
			};
...
var t$2 = globalThis.EventTarget;
		...
		try {
			while (true) {
				...
				try {
					...
					if (e$2 === (EXPLICIT_ABORT_TOKEN ||= createExplicitAbortToken())) return;
					else throw e$2;
				}
			}
		} finally {
...
function createPendingRpcSubscription(transport, subscriptionsPlan) {
	return { async subscribe({ abortSignal }) {
...
function getRpcSubscriptionsChannelWithAutoping({ abortSignal: callerAbortSignal, channel, intervalMs }) {
	let intervalId;
	function sendPing() {
		channel.send(PING_PAYLOAD).catch((e$2) => {
			if (isSolanaError(e$2, SOLANA_ERROR__RPC_SUBSCRIPTIONS__CHANNEL_CONNECTION_CLOSED)) pingerAbortController.abort();
		});
	}
...
function getRpcSubscriptionsTransportWithSubscriptionCoalescing(transport) {
	...
	return function rpcSubscriptionsTransportWithSubscriptionCoalescing(config) {
		...
		let cachedDataPublisherPromise = cache$1.get(subscriptionConfigurationHash);
		if (!cachedDataPublisherPromise) {
			...
			dataPublisherPromise.then((dataPublisher) => {
				dataPublisher.on("error", () => {
					cache$1.delete(subscriptionConfigurationHash);
					abortController.abort();
				}, { signal: abortController.signal });
			}).catch(() => {});
			cache$1.set(subscriptionConfigurationHash, cachedDataPublisherPromise = {
				abortController,
...
async function createSignerFromKeyPair(keyPair) {
	...
	const out = {
		address: address$1,
		keyPair,
		signMessages: (messages) => Promise.all(messages.map(async (message) => Object.freeze({ [address$1]: await signBytes(keyPair.privateKey, message.content) }))),
		signTransactions: (transactions) => Promise.all(transactions.map(async (transaction) => {
			...
			return Object.freeze({ [address$1]: signedTransaction.signatures[address$1] });
		}))
	};
...
var associatedTokenErrorMessages = { [ASSOCIATED_TOKEN_ERROR__INVALID_OWNER]: `Associated token account owner does not match address derivation` };
...
function getTransferInstructionDataEncoder() {
	return transformEncoder(getStructEncoder([["discriminator", getU8Encoder()], ["amount", getU64Encoder()]]), (value) => ({
		...value,
...
//#endregion
//#region node_modules/@privy-io/react-auth/dist/esm/use-sign-with-user-signer-BYctG7hm.mjs
var import_react = require_react();
var r = () => {
	let { walletProxy: r$1, client: a } = I();
	return (0, import_react.useMemo)((() => ({ signWithUserSigner: async ({ message: t$3, targetAppId: e$2 }) => {
		if (!r$1) throw Error("Wallet proxy not initialized");
		let i = await a.getAccessToken();
		if (!i) throw Error("User must be authenticated");
		let { signature: s } = await r$1.signWithUserSigner({
			accessToken: i,
			message: t$3,
			targetAppId: e$2
		});
...
//#endregion
export { getU8Decoder as $, mainnet as A, getEnumDecoder as B, decompileTransactionMessage as C, setTransactionMessageFeePayer as D, prependTransactionMessageInstruction as E, getArrayDecoder as F, getUnitDecoder as G, getMapDecoder as H, getBooleanDecoder as I, getU16Decoder as J, getF64Decoder as K, getBytesDecoder as L, upgradeRoleToSigner as M, pipe as N, setTransactionMessageLifetimeUsingBlockhash as O, getOptionDecoder as P, getU64Encoder as Q, getConstantDecoder as R, createTransactionMessage as S, isTransactionMessageWithDurableNonceLifetime as T, getStructDecoder as U, getHiddenPrefixDecoder as V, getStructEncoder as W, getU32Encoder as X, getU32Decoder as Y, getU64Decoder as Z, getBase64EncodedWireTransaction as _, transformEncoder as _t, getTransferInstruction as a, assertAccountExists as at, appendTransactionMessageInstruction as b, SolanaError as bt, createKeyPairSignerFromPrivateKeyBytes as c, getBase58Decoder as ct, isTransactionSigner as d, getBase64Encoder as dt, getU8Encoder as et, partiallySignTransactionMessageWithSigners as f, getUtf8Decoder as ft, compileTransaction as g, padLeftEncoder as gt, createSolanaRpc as h, fixDecoderSize as ht, getCreateAssociatedTokenIdempotentInstruction as i, getProgramDerivedAddress as it, AccountRole as j, devnet as k, isTransactionModifyingSigner as l, getBase58Encoder$1 as lt, createSolanaRpcSubscriptions as m, containsBytes as mt, TOKEN_PROGRAM_ADDRESS as n, getAddressDecoder as nt, fetchAddressesForLookupTables as o, decodeAccount as ot, setTransactionMessageFeePayerSigner as p, addDecoderSizePrefix as pt, getI16Decoder as q, findAssociatedTokenPda as r, getAddressEncoder as rt, createKeyPairSignerFromBytes as s, fetchEncodedAccount as st, r as t, address as tt, isTransactionPartialSigner as u, getBase64Decoder as ut, getTransactionDecoder as v, SOLANA_ERROR__TRANSACTION__FAILED_TO_ESTIMATE_COMPUTE_LIMIT as vt, getCompiledTransactionMessageDecoder as w, appendTransactionMessageInstructions as x, isSolanaError as xt, getTransactionEncoder as y, SOLANA_ERROR__TRANSACTION__FAILED_WHEN_SIMULATING_TO_ESTIMATE_COMPUTE_LIMIT as yt, getDiscriminatedUnionDecoder as z };
//# sourceMappingURL=use-sign-with-user-signer-BYctG7hm-DxXkjQWt.js.map
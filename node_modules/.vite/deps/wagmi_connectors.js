import { s as __toDynamicImportESM } from "./chunk-CBgdiQcn.js";
import { Da as hexToNumber, Gr as ResourceUnavailableRpcError, qr as SwitchChainError, sa as getAddress, ti as UserRejectedRequestError, xa as numberToHex } from "./ccip-D4p2e5rB.js";
import { Zn as withRetry, sn as withTimeout } from "./_esm-CRgaECpT.js";
...
import { mt as ChainNotConfiguredError, y as ProviderNotFoundError } from "./writeContract-DTpQ9_6D.js";
import { h as createConnector, m as injected, p as mock, t as extractRpcUrls } from "./exports-CS2Su7Ol.js";
		...
		async getProvider() {
			if (!walletProvider) {
				...
				const { createBaseAccountSDK } = await (() => {
					try {
						return import("./dist-CqyCIFUU.js");
					} catch {
		...
		async switchChain({ addEthereumChainParameter, chainId }) {
			...
			try {
				...
				if (error.code === 4902) try {
					...
					throw new UserRejectedRequestError(error$1);
				}
		...
		async getProvider() {
			if (!walletProvider) {
				const { createCoinbaseWalletSDK } = await (() => {
					try {
						return import("./dist-CrLTgxb6.js");
					} catch {
		...
		async getProvider() {
			if (!walletProvider) {
				const { GeminiWalletProvider } = await (() => {
					try {
						return import("./dist-D3qtzab2.js");
					} catch {
...
function metaMask(parameters = {}) {
	...
	return createConnector((config) => ({
		...
		type: metaMask.type,
		async setup() {
			const provider$1 = await this.getProvider();
			if (provider$1?.on) {
				if (!connect) {
					connect = this.onConnect.bind(this);
					provider$1.on("connect", connect);
				}
				if (!accountsChanged) {
					accountsChanged = this.onAccountsChanged.bind(this);
					provider$1.on("accountsChanged", accountsChanged);
				}
			}
		},
		async connect({ chainId, isReconnecting, withCapabilities } = {}) {
			...
			if (!displayUri) {
				...
				provider$1.on("display_uri", displayUri);
			}
			let accounts = [];
			...
			try {
				...
				if (displayUri) {
					provider$1.removeListener("display_uri", displayUri);
					displayUri = void 0;
				}
				if (signResponse) provider$1.emit("connectAndSign", {
					accounts,
					chainId: currentChainId,
					signResponse
				});
				else if (connectWithResponse) provider$1.emit("connectWith", {
					accounts,
					chainId: currentChainId,
					connectWithResponse
				});
				if (connect) {
					provider$1.removeListener("connect", connect);
					connect = void 0;
				}
				if (!accountsChanged) {
					accountsChanged = this.onAccountsChanged.bind(this);
				...
				if (!chainChanged) {
					chainChanged = this.onChainChanged.bind(this);
					provider$1.on("chainChanged", chainChanged);
				}
				if (!disconnect) {
					disconnect = this.onDisconnect.bind(this);
					provider$1.on("disconnect", disconnect);
				}
				return {
					accounts: withCapabilities ? accounts.map((address) => ({
						address,
		...
		async disconnect() {
			const provider$1 = await this.getProvider();
			if (chainChanged) {
				provider$1.removeListener("chainChanged", chainChanged);
				chainChanged = void 0;
			}
			if (disconnect) {
				provider$1.removeListener("disconnect", disconnect);
				disconnect = void 0;
			}
			if (!connect) {
				connect = this.onConnect.bind(this);
		...
		async getChainId() {
			const provider$1 = await this.getProvider();
			const chainId = provider$1.getChainId() || await provider$1?.request({ method: "eth_chainId" });
			return Number(chainId);
		},
		async getProvider() {
			async function initProvider() {
				const MetaMaskSDK = await (async () => {
					const { default: SDK } = await (() => {
						try {
							return import("./metamask-sdk-CqLjQeTw.js");
						} catch {
				...
				if (!provider$1) throw new ProviderNotFoundError();
				return provider$1;
			}
			if (!provider) {
		...
		async switchChain({ addEthereumChainParameter, chainId }) {
			...
			try {
				...
				if (error.code === 4902 || error?.data?.originalError?.code === 4902) try {
					...
					const error$1 = err$1;
					if (error$1.code === UserRejectedRequestError.code) throw new UserRejectedRequestError(error$1);
					throw new SwitchChainError(error$1);
				}
				throw new SwitchChainError(error);
			}
			async function waitForChainIdToSync() {
				await withRetry(async () => {
					const value = hexToNumber(await provider$1.request({ method: "eth_chainId" }));
			...
			async function sendAndWaitForChangeEvent(chainId$1) {
				await new Promise((resolve) => {
					const listener = ((data) => {
						if ("chainId" in data && data.chainId === chainId$1) {
							config.emitter.off("change", listener);
							resolve();
						}
					});
					config.emitter.on("change", listener);
					config.emitter.emit("change", { chainId: chainId$1 });
				});
			}
		},
		...
		async onDisconnect(error) {
			...
			if (error && error.code === 1013) {
				if (provider$1 && !!(await this.getAccounts()).length) return;
			}
			config.emitter.emit("disconnect");
			if (chainChanged) {
				provider$1.removeListener("chainChanged", chainChanged);
				...
				try {
					if (!accounts?.length && !isReconnecting) {
						const { RpcSchema } = await (() => {
							try {
								return import("./dist-DPZxa9Yw.js");
							} catch {
						...
						const { z } = await (() => {
							try {
								return import("./internal-RGnPuPdD.js");
							} catch {
			...
			if (!provider_) {
				...
				provider_ = new (await ((async () => {
					const Provider = await (() => {
						try {
							return import("./dist-DN-a-LHL.js").then(__toDynamicImportESM(1));
						} catch {
		...
		async getProvider({ chainId } = {}) {
			async function initProvider() {
				...
				const { EthereumProvider } = await (() => {
					try {
						return import("./dist-DKAZgbew.js");
					} catch {
